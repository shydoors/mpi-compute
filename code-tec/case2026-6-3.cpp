...
#pragma omp parallel for schedule(static)
        for (int i = 0; i < N; ++i) {
          buf[i] = std::cos(i * 0.001 + c);
        }
      }
      MPI_Isend(buf.data(), N, MPI_DOUBLE, rank + 1, c, MPI_COMM_WORLD,
                &req_send);
      send_pending = true;
    }
    if (rank > 0 && c < CHUNKS - 1) { // 提前投递下一块接收请求
      MPI_Irecv(buf.data(), N, MPI_DOUBLE, rank - 1, c + 1, MPI_COMM_WORLD,
                &req_recv);
      recv_pending = true;
    }
  }
