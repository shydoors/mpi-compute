...
int main(int argc, char **argv) {
  // 1. 初始化混合并行环境
  // 声明
  // MPI_THREAD_FUNNELED：仅主线程调用MPI，Worker线程纯计算，符合生产安全规范
  int provided;
  ...
  for (int iter = 0; iter < MAX_ITER; ++iter) {
    ...
    MPI_Status status;
    MPI_Sendrecv(&u[N_LOCAL + HALO - 1], 1, MPI_DOUBLE, right_rank, 0, &u[0], 1,
                 MPI_DOUBLE, left_rank, 0, MPI_COMM_WORLD, &status);
    MPI_Sendrecv(&u[HALO], 1, MPI_DOUBLE, left_rank, 0, &u[N_TOTAL - 1], 1,
                 MPI_DOUBLE, right_rank, 0, MPI_COMM_WORLD, &status);
