...
int main(int argc, char **argv) {
  ...
  int local_output_start =
      world_rank * base_output + std::min(world_rank, remainder);

  // 计算需要的信号范围（含 halo）
  int sig_start = std::max(0, local_output_start - halo);
  int sig_end =
      std::min(GLOBAL_SIGNAL_LENGTH, local_output_start + local_output_len);
  ...
  if (world_rank == 0) {
    ...
    for (int r = 0; r < world_size; ++r) {
      ...
      int r_sig_end =
          std::min(GLOBAL_SIGNAL_LENGTH, r_output_start + r_output_len);
    ...
    for (int r = 0; r < world_size; ++r) {
      std::cout << "  进程 " << r << ": 信号 [" << send_displs[r] << ", "
                ...
                << " 长度=" << send_counts[r] << " 输出 ["
                << (r * base_output + std::min(r, remainder)) << ", "
                << (r * base_output + std::min(r, remainder) + base_output +
                    (r < remainder ? 1 : 0) - 1)
                << "]\n";
    }
    std::cout << "\n";
  ...
  MPI_Scatterv(global_signal.data(), send_counts.data(), send_displs.data(),
               MPI_INT, local_signal.data(), local_signal_len, MPI_INT, 0,
               MPI_COMM_WORLD);
  ...
  MPI_Gatherv(local_output.data(), local_output_len, MPI_INT,
              global_output.data(), out_counts.data(), out_displs.data(),
