...
// 共轭梯度法求解 Ax = b
vector<double> conjugateGradient(
    ...
    for (int k = 0; k < maxIter; ++k) {
        ...
        cout << "迭代 " << k + 1 << ": 残差范数 = "
            << fixed << setprecision(10) << sqrt(rsnew) << endl;
...
// 验证解：计算残差 ||Ax - b||
double verifySolution(const vector<vector<double>>& A,
...
int main() {
    ...
    cout << "残差范数 ||Ax - b|| = " << fixed << setprecision(10) << residual << endl;
