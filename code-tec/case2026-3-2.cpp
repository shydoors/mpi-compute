...
int main(int argc, char** argv)
{
    ...
    // 使用 Scatterv 分发数据
    MPI_Scatterv(
        rank == 0 ? A_flat_full.data() : nullptr, counts.data(), displs.data(), MPI_DOUBLE,
        A_flat_local.data(), counts[rank] * N, MPI_DOUBLE,
        0, MPI_COMM_WORLD
    );

    MPI_Scatterv(
        rank == 0 ? b_full.data() : nullptr, counts.data(), displs.data(), MPI_DOUBLE,
        b_local.data(), counts[rank], MPI_DOUBLE,
        0, MPI_COMM_WORLD
    );
    ...
    if (rank == 0) cout << "迭代开始..." << endl;

    for (int k = 0; k < maxIter; ++k) {
        // --- 步骤 1: 收集完整的 p 向量到所有进程 (关键并行通信) ---
        MPI_Allgatherv(
            p_local.data(), counts[rank], MPI_DOUBLE,
            p_global.data(), counts.data(), displs.data(), MPI_DOUBLE,
            MPI_COMM_WORLD
        );
        ...
        MPI_Allreduce(&rsnew_local, &rsnew_global, 1, MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD);

        if (rank == 0) {
            cout << "迭代 " << k + 1 << ": 残差范数 = "
                << fixed << setprecision(10) << sqrt(rsnew_global) << endl;
        }

        if (sqrt(rsnew_global) < tol) {
            if (rank == 0) cout << "在 " << k + 1 << " 次迭代后收敛。" << endl;
            break;
        }
    ...
    MPI_Gatherv(
        x_local.data(), counts[rank], MPI_DOUBLE,
    ...
    if (rank == 0) {
        ...
        cout << "最终残差范数 ||Ax - b|| = " << fixed << setprecision(10) << sqrt(residual_sq) << endl;
        cout << "解向量前 5 个元素：";
