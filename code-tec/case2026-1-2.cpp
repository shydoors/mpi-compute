...
int main(int argc, char** argv) {
    ...
    if (rank == 0) {
        ...
        MPI_Recv(localMat1.data(), my_rows * cols, MPI_INT, 0, 1, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
        MPI_Recv(localMat2.data(), my_rows * cols, MPI_INT, 0, 1, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
    }
    ...
    if (rank == 0) {
        ...
        for (int p = 1; p < size; ++p) {
            ...
            MPI_Recv(globalResult.data() + offset, count, MPI_INT, p, 2, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
            offset += count;
        }
