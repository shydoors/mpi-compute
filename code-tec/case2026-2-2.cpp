...
int main(int argc, char **argv) {
  ...
  int my_rows =
      base_rows + (rank == size - 1 ? remainder : 0); // 当前进程处理的A的行数
  ...
  if (rank == 0) {
    ...
    for (int p = 1; p < size; ++p) {
      ...
      MPI_Send(flatA.data() + offset, count, MPI_INT, p, 1,
               MPI_COMM_WORLD); // tag=1: A数据
      offset += count;
    }
  } else {
    MPI_Recv(localA.data(), my_rows * n, MPI_INT, 0, 1, MPI_COMM_WORLD,
             MPI_STATUS_IGNORE);
  }
  ...
  if (rank == 0) {
    ...
    // 向其他进程发送完整的 B
    for (int p = 1; p < size; ++p) {
      MPI_Send(flatB.data(), n * n, MPI_INT, p, 3,
               MPI_COMM_WORLD); // tag=3: B数据
    }
  } else {
    MPI_Recv(fullB.data(), n * n, MPI_INT, 0, 3, MPI_COMM_WORLD,
             MPI_STATUS_IGNORE);
  }
  ...
  if (rank == 0) {
    ...
    for (int p = 1; p < size; ++p) {
      int proc_rows = base_rows + (p == size - 1 ? remainder : 0);
      int count = proc_rows * n;
      MPI_Recv(globalC.data() + offset, count, MPI_INT, p, 2, MPI_COMM_WORLD,
               MPI_STATUS_IGNORE); // tag=2: 结果
      offset += count;
    }
    ...
    cout << "Time taken by matrix multiplication: " << duration.count()
         << " microseconds" << endl;
