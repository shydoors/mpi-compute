 ...
 * 编译：mpicxx -o edge_detect edge_detect.cpp `pkg-config --cflags --libs opencv4`
 * 运行：mpiexec -n 4 ./edge_detect
 */
...
int main(int argc, char** argv)
{
    ...
    DEBUG_PRINT("本地分配: " << local_rows << " 行, 数据量: " << sendcounts[rank] << " 字节");
    ...
    if (rank > 0) {
        ...
        MPI_Sendrecv(
            ...
            local_buffer + width, width, MPI_UNSIGNED_CHAR,
            up_rank, 100,  // Tag 100: 发送有效数据行
            // 接收：上游的最后1行 → 我的 Top Halo
            local_buffer, width, MPI_UNSIGNED_CHAR,
            up_rank, 101,  // Tag 101: 接收 Halo 数据
    ...
    if (rank < size - 1) {
        ...
        uchar* my_last_row = local_buffer + width + (local_rows - 1) * width;  // 我的最后有效行
        uchar* my_bottom_halo = local_buffer + (local_rows + 1) * width;        // 我的 Bottom Halo 位置

        MPI_Sendrecv(
            // 发送：我的最后1行有效数据 → 下游的 Top Halo
            my_last_row, width, MPI_UNSIGNED_CHAR,
            down_rank, 101,  // Tag 101
            // 接收：下游的第1行 → 我的 Bottom Halo
            my_bottom_halo, width, MPI_UNSIGNED_CHAR,
            down_rank, 100,  // Tag 100
            MPI_COMM_WORLD, MPI_STATUS_IGNORE
        );
    }
    else {
    ...
    MPI_Gatherv(
        result_buffer + width,        // ⚠️ 发送指针偏移（跳过 Top Halo）
        sendcounts[rank], MPI_UNSIGNED_CHAR,
