...
void run_worker() {
  while (true) {
    ...
    MPI_Recv(&task, sizeof(Task), MPI_BYTE, MASTER_RANK, MPI_ANY_TAG,
...
#pragma omp parallel for reduction(+ : sum) schedule(dynamic)
    ...
    MPI_Send(&res, sizeof(Result), MPI_BYTE, MASTER_RANK, TAG_RESULT,
             MPI_COMM_WORLD);
  }
}
...
void run_master(int world_size) {
  ...
  while (!idle_workers.empty() && dispatched < total_tasks) {
    ...
    idle_workers.pop();
    MPI_Send(&tasks[dispatched], sizeof(Task), MPI_BYTE, w, TAG_WORK,
             MPI_COMM_WORLD);
    dispatched++;
  }
  ...
  while (completed < total_tasks) {
    Result res;
    MPI_Status status;
    MPI_Recv(&res, sizeof(Result), MPI_BYTE, MPI_ANY_SOURCE, TAG_RESULT,
             MPI_COMM_WORLD, &status);
    std::cout << "[Master] 任务 " << res.task_id << " 完成 | 来源: Rank "
              << status.MPI_SOURCE
...
int main(int argc, char **argv) {
  ...
  if (size < 2) {
    if (rank == MASTER_RANK)
{      std::cerr << "错误：至少需要 1 Master + 1 Worker\n";
}    MPI_Finalize();
    return 1;
  }
  if (rank == MASTER_RANK) {
