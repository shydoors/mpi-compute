...
int main(int argc, char** argv) {
    ...
    if (size < 3) {
        if (rank == 0) std::cerr << "功能分解需至少 3 个 MPI 进程\n";
...
#pragma omp parallel for schedule(static)
            ...
            // 【模块B：核心求解器 / 物理计算】
            MPI_Recv(buf.data(), N, MPI_DOUBLE, 0, step, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
...
#pragma omp parallel for schedule(static)
            ...
            // 【模块C：后处理 / 统计监控】
            MPI_Recv(buf.data(), N, MPI_DOUBLE, 1, step, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
