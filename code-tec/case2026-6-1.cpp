...
int main(int argc, char **argv) {
  ...
  while (iter < max_iter && diff > tol) {
    // ================= MPI 层：跨节点 Halo 交换 =================
    MPI_Sendrecv(&u[1 * N], N, MPI_DOUBLE, up_rank, 0, &u[0 * N], N, MPI_DOUBLE,
                 up_rank, 1, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
...
#pragma omp parallel for reduction(max : local_max_diff) schedule(static)
    ...
    // ================= MPI 层：全局收敛同步 =================
    MPI_Allreduce(&local_max_diff, &diff, 1, MPI_DOUBLE, MPI_MAX,
                  MPI_COMM_WORLD);
