...
int main(int argc, char **argv) {
  ...
  MPI_Scatterv(flatMat1.data(), sendcounts, displs, MPI_INT, localMat1.data(),
               recvcount, MPI_INT, 0, MPI_COMM_WORLD);
  MPI_Scatterv(flatMat2.data(), sendcounts, displs, MPI_INT, localMat2.data(),
               recvcount, MPI_INT, 0, MPI_COMM_WORLD);
  ...
  MPI_Gatherv(localResult.data(), recvcount, MPI_INT, globalResult.data(),
  ...
  if (rank == 0) {
    ...
    for (int i = 0; i < rows; ++i) {
      copy(globalResult.begin() + i * cols,
           globalResult.begin() + (i + 1) * cols, resultMatrix[i].begin());
    }

    // 【修改】如果行数 <= 5，打印结果矩阵
    if (rows <= 5) {
      cout << "===== Rank 0: Result Matrix =====" << endl;
      cout << "Resultant matrix after addition (" << rows << "x" << cols
           << "):" << endl;
    ...
    cout << "[Rank 0] Time taken by matrix addition: " << duration.count()
         << " microseconds"
