# 语义准确

1. 使用 `cstdint` 库来声明整数类型
* `std::int32_t` ==> `i32`
* `std::uint32_t` ==> `u32`
* `std::int64_t` ==> `i64`
* `std::uint64_t` ==> `u64`
2. 文件路径
* 文件路径类型 `char*` >`std::filesystem::path` > `std::string`。
* 读写文件时，不得使用文件的绝对地址，只准用相对地址。项目目录作为根目录。
3. 防止逻辑歧义
* 类的初始化使用大括号（统一列表初始化）。
* 不得省略括号、大括号。尤其 for 循环、if 语句判断的时候。
* 禁止全局使用命名空间（`using namespace xxx;`），显式调用对应命名空间的函数。
* 优先使用 `const` 和 `constexpr`，避免使用 `#define` 宏污染。
4. 智能指针优先于裸指针，避免手动 `new/delete`。
5. 函数参数传递：非基本类型使用 `const &`（只读）或 `&`（可写），避免不必要的拷贝。

# 版本

| KEY               | VALUE                     |
|:------------------|:--------------------------|
| C++ 标准            | C++17                     |
| OpenMP            | >= 4.5                    |
| CMake             | >= 3.16                   |
| nvcc 主机侧（gcc/g++） | gcc (GCC) 16.1.1 20260430 |
| nvcc 主机侧（msvc）    | VS2019                    |

# 编译器参数

| KEY     | VALUE                                      |
|:--------|:-------------------------------------------|
| DEBUG   | `-g -O0 -std=c++17 -Wall -Wextra -fopenmp` |
| RELEASE | `-O2 -std=c++17 -Wall -Wextra -fopenmp`    |

# 项目结构

* `src/` — 源文件
* `include/` — 头文件
* `build/` — 构建产物（不提交到 git）
* `code-stu/` — 学习代码、课本示例（自动扫描编译）
* `code-tec/` — 学习代码、老师提供（自动扫描编译）
* `data/` — 输入数据
* `results/` — 输出文件，以时间戳为文件夹（不提交到 git）
* `docs/` — 文档
* `docs/books` — 转为md的pdf课件
* `script/` — 脚本，包括但不限于预处理文件
# 文档编写要求

1. 不得使用任何无法用输入法打出的字符
2. 默认utf-8