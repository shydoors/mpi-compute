...
```
1. 初始化 K 个中心 $\mu_1, \mu_2, \ldots, \mu_K$
2. 重复直到收敛：
   a. 分配：对每个点 $x_i$，分配到最近的中心
      $c(i) = \arg\min_j \| x_i - \mu_j \|^2$
   b. 更新：重新计算每个簇的中心
      $\mu_j = (1/|C_j|) \cdot \sum_{i \in C_j} x_i$
   c. 如果中心移动量 < 阈值，则收敛
```
...
每个 block 动态分配共享内存：
- 中心点副本：$k \times 2 \times \text{sizeof(f64)}$ 字节
- 归约暂存：$\text{blockDim.x} \times (\text{sizeof(f64)} + \text{sizeof(u32)})$ 字节
