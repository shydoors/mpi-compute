...
// Kernel: 分配 + 局部聚合一趟完成
// ============================================================
__global__ void assign_kernel_stream(
...
KMeansResult KMeans::run_cuda(Dataset& data, u32 k) {
    ...
    auto get_y = [=](u64 i) -> f64 { return interleaved ? raw[i*2+1] : ys[i]; };
    ...
    std::printf("  [CUDA] Chunk: %llu 点, max %llu blocks\n",
                ...
                (unsigned long long)((pts + 255) / 256));
    ...
    for (i32 iter = 0; iter < config_.max_iterations; ++iter) {
        ...
        while (offset < n) {
            ...
            cudaMemcpy(d_x, chunk_x.get(), chunk * sizeof(f64), cudaMemcpyHostToDevice);
            cudaMemcpy(d_y, chunk_y.get(), chunk * sizeof(f64), cudaMemcpyHostToDevice);
            ...
            assign_kernel_stream<<<blocks, 256, shmem>>>(
                ...
                d_sumx, d_sumy, d_cnt, chunk, k);
            cudaDeviceSynchronize();

            // 回传标签
            cudaMemcpy(result.labels.get() + offset, d_labels,
        ...
        if ((iter + 1) % 5 == 0 || iter == 0)
            std::printf("  [CUDA] Iter %3d / %d  delta=%.6e\n",
                        iter + 1, config_.max_iterations, delta);
    ...
    cudaFree(d_x); cudaFree(d_y); cudaFree(d_labels);
    cudaFree(d_cx); cudaFree(d_cy);
    cudaFree(d_sumx); cudaFree(d_sumy); cudaFree(d_cnt);
