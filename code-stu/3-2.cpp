...
int main(void) {
  ...
  if (0 == rank) {
    ...
    MPI_Recv(&data_b, size, MPI_INT, 0, tag, MPI_COMM_WORLD, &stat);
    printf("rank %d got %d\n", rank, data_b);
  }
  MPI_Finalize();
  return 0;
 ...
 * - count: 要接收的数据项的最大数量。
 *  改成 2
 * 能接收两个数据项，但是能不能接收成功还要看发送方发送了多少数据项，如果发送方只发送了一个数据项，那么接收方只能接收到一个数据项，剩余的一个数据项将不会被接收。
 * - datatype:
 * 数据项的类型（如MPI_INT、MPI_FLOAT等）。注意，结构体类型需要使用MPI_Type_create_struct函数创建自定义数据类型。
